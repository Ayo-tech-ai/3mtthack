...
for i in range(0, len(models), cols_per_row):
    cols = st.columns(cols_per_row, gap="large")  # Slightly larger spacing
    for j, model in enumerate(models[i:i+cols_per_row]):
        with cols[j]:
            st.markdown(
                f"""
                <div style='
                    height: 230px;  /* Uniform card height */
                    padding: 20px; 
                    border-radius: 15px; 
                ...
                ' onmouseover="this.style.transform='translateY(-5px)'; this.style.boxShadow='0 8px 25px rgba(0,0,0,0.2)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='3px 3px 20px rgba(0,0,0,0.15)';">
                    <div>
                        <h3 style='margin:0'>{model['label']}</h3>
                        <p style='font-size:14px; color:#333; margin-top:5px;'>{model['desc']}</p>
                    </div>
                    <div>
